...
class CBSState:
    ...
    def compute_cost(self):
        ...
        for agent in range(len(self._starts)):
        ...
        for path in self._paths.values():
            for node in path:
                if len(no_conflict) == 0:
                    no_conflict.append(node)
                elif node not in no_conflict:
                        no_conflict.append(node)
                elif node.__eq__(no_conflict[no_conflict.index(node)]):
                    return False, (node, node.get_g())
        return True, None
    ...
    def successors(self):
        ...
        for i in range(self._k):
            for node in self._paths.get(i):
                if node.__eq__(state[0]):
                    if i not in agent_conflict:
                        agent_conflict.append(i)
        
        for i in agent_conflict[:2]:
            c = CBSState(self._map, self._starts, self._goals)
            c._constraints = copy.deepcopy(self._constraints)
            c.set_constraint(state[0], state[1], i)
            states.append(c)
